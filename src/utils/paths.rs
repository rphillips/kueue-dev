...
pub fn get_operator_source_path() -> Result<Option<PathBuf>> {
...
pub fn ensure_operator_source_directory() -> Result<PathBuf> {
    let source_path = get_operator_source_path()?.ok_or_else(|| {
        let config_hint = if let Some(path) = Settings::active_config_file() {
            format!(
                "Please set kueue_operator_source_path in your configuration file:\n{}\n",
                path.display()
            )
        } else {
            "Please set kueue_operator_source_path in your configuration file.\n".to_string()
...
pub fn operator_source_join(relative_path: &str) -> PathBuf {
    if let Ok(Some(source_path)) = get_operator_source_path() {
